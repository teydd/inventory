  ...
  return (
    ...
    <div className='fixed left-0 top-0 bg-cyan-900 text-white w-64 min-h-screen p-6'>
        ...
        {navigation.map((item,key)=>{
            ...
            return (
            <Link className={`flex items-center space-x-3 py-2 rounded-xl px-3 ${isActive ? "bg-cyan-100 text-gray-800":"hover:bg-gray-800 text-gray-300"}`} href={item.href} key={key}><Icon className='w-5 h-5'></Icon><span className='text-sm'> {item.name}</span></Link>
        )
        })}
    </nav>
